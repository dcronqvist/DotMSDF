...
{
    public static int SolveQuadratic(ref double[] x, double a, double b, double c)
    {
        // a == 0 -> linear equation
    ...
    private static int SolveCubicNormed(ref double[] x, double a, double b, double c)
    {
        double a2 = a * a;
    ...
    public static int SolveCubic(ref double[] x, double a, double b, double c, double d)
    {
        if (a != 0)
        {
            double bn = b / a;
            if (Math.Abs(bn) < 1e6) // Above this ratio, the numerical error gets larger than if we treated a as zero
                return SolveCubicNormed(ref x, bn, c / a, d / a);
        }
